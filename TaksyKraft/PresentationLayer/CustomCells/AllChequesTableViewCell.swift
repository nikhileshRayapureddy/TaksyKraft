//
//  AllChequesTableViewCell.swift
//  TaksyKraft
//
//  Created by Nikhilesh on 01/03/18.
//  Copyright © 2018 TaksyKraft. All rights reserved.
//

import UIKit
protocol AllChequesTableViewCellDelegate
{
    func btnRejectClicked(chqId : String,cell : AllChequesTableViewCell)
    func btnApproveClicked(chqId : String,cell : AllChequesTableViewCell)
    func btnImgClicked(chqBO : ChequeBO,cell : AllChequesTableViewCell)
}

class AllChequesTableViewCell: UITableViewCell {

    @IBOutlet weak var lblAmt: UILabel!
    @IBOutlet weak var lblID: UILabel!
    
    @IBOutlet weak var lblEmpName: UILabel!
    
    @IBOutlet weak var lblReason: UILabel!
    @IBOutlet weak var lblUploadedDate: UILabel!
    @IBOutlet weak var lblEmpId: UILabel!
    
    @IBOutlet weak var lblChequeNo: UILabel!
    @IBOutlet weak var lblClearanceDate: UILabel!    
    @IBOutlet weak var lblDesc: UILabel!
    @IBOutlet weak var vwBtnBase: UIView!
    @IBOutlet weak var lblStatus: UILabel!
    
    @IBOutlet weak var btnApprove: UIButton!
    
    @IBOutlet weak var lblAttachsCount: UILabel!
    @IBOutlet weak var btnImg: UIButton!
    @IBOutlet weak var btnReject: UIButton!
    @IBOutlet weak var imgCheque: UIImageView!
    @IBOutlet weak var constLblReasonHeight: NSLayoutConstraint!
    @IBOutlet weak var constLblDescriptionHeight: NSLayoutConstraint!
    @IBOutlet weak var constVwBtnBaseHeight: NSLayoutConstraint!
    var chqBO = ChequeBO()
    var callBack : AllChequesTableViewCellDelegate!
    override func awakeFromNib() {
        super.awakeFromNib()
        // Initialization code
    }

    override func setSelected(_ selected: Bool, animated: Bool) {
        super.setSelected(selected, animated: animated)

        // Configure the view for the selected state
    }

    @IBAction func btnApproveClicked(_ sender: UIButton) {
        if callBack != nil{
            callBack.btnApproveClicked(chqId: chqBO.chequeId, cell: self)
        }
    }
    
    @IBAction func btnRejectClicked(_ sender: UIButton) {
        if callBack != nil{
            callBack.btnRejectClicked(chqId: chqBO.chequeId, cell: self)
        }
    }
    
    @IBAction func btnImgClicked(_ sender: UIButton) {
        if callBack != nil{
            callBack.btnImgClicked(chqBO: chqBO, cell: self)
        }
    }
}
